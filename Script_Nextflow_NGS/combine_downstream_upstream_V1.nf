...
process Target_first {
    ...
    script:
    """
    Target_First.py "${sample_file}" --mount-dir "${params.output_dir}/basespace"
    echo "complete" > FEV2_bs_status_complete.txt
    """
...
process Indiegene_CE_som {
    ...
    echo "complete" > INDIEGENE_bs_status_complete.txt
    """
...
workflow {
    ...
    target_first_out =Target_first(preprocessing_for_launch.out, copy_setup)
    indiegene_ce_out =Indiegene_CE_som(preprocessing_for_launch.out, copy_setup)
    ...
    comb_comp_marker = target_first_out.mix(indiegene_ce_out)
    //QC(params.output_dir, preprocessing_for_launch.out, copy_setup, comb_comp_marker)
    CNV_FeV2(params.output_dir, preprocessing_for_launch.out, copy_setup, comb_comp_marker)
    //CNV_Indiegene(params.output_dir, preprocessing_for_launch.out, copy_setup, comb_comp_marker)
    //DNA_fusion(params.output_dir, preprocessing_for_launch.out, copy_setup, comb_comp_marker)
    //Hotspot(params.output_dir, preprocessing_for_launch.out, copy_setup, comb_comp_marker)
    //Gene_Coverage(params.output_dir, preprocessing_for_launch.out, copy_setup, comb_comp_marker)
}
