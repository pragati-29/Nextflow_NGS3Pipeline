...
df_main_sample['Combined_info'] = (
    df_main_sample['#chrom'].astype(str)
    + ":" + df_main_sample['start'].astype(str)
    + ":" + df_main_sample['end'].astype(str)
    + ":" + df_main_sample['gene_name']
)
...
df_merged = pd.merge(
    df_main_sample_filt,
    df_baseline_filt,
    on=['#chrom', 'start', 'end', 'gene_name', 'Combined_info'],
    how='left'
)
...
if csv_file and txt_file:
    ...
    # Merge on 'Gene' vs 'Gene Name'
    results = file1.merge(file2, left_on='Gene', right_on='Gene Name', how='left')

    # Rename the three relevant columns in the final output to match the "4bc_cohort_" convention
    results.rename(
        columns={
    ...
    # Rebuild the final DataFrame with the renamed columns in the same relative positions
    results = results[
        [
            'Chromosome', 'Start', 'End', 'Predicted_copy_number',
            'Type_of_alteration', 'MedianRatio', 'Gene',
            '4bc_cohort_DAZ_Score', '4bc_cohort_DAP_Value', '4bc_cohort_DA_Percentile'
        ]
    ]

    # Format numeric columns as needed (DAP_Value)
